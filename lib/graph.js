 ...
 * @returns {Boolean} true is node found and destroyed
 */
graph.prototype.remove_node = function(node_id){
...
graph.prototype.remove_edge = function(subject_id, object_id, predicate_id){
	...
	// For the next little bit, keep an eye on whether these are
	// unique parts of the graph.
	var is_unique_sub_p = false;
	var is_unique_obj_p = false;
	var is_unique_pred_p = false;

	// Does this subject appear elsewhere? Decrement or eliminate
	// as necessary.
	if( this._subjects[ subject_id ] == 1 ){
	    delete this._subjects[ subject_id ];
	    is_unique_sub_p = true;
	}else{
	...
	if( ! this._subjects[ subject_id ] && ! this._objects[ subject_id ] ){
	    this._is_a_singleton_lookup[ subject_id ] = true;
	}
	// Was the object singletoned?
	if( ! this._subjects[ object_id ] && ! this._objects[ object_id ] ){
